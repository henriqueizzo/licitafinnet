"""Geração de declarações de habilitação em Word (.docx) com a identidade Finnet.
...
AZUL_FINNET = RGBColor(0x1C, 0x2E, 0x55)
...
def _instrucao_ia(licitacao: Licitacao, perfil: PerfilEmpresa, documento: str, referencia: str) -> str:
    empresa = perfil.razao_social or "FINNET S/A - TECNOLOGIA E INSTITUIÇÃO DE PAGAMENTO"
...
def gerar_docx(licitacao: Licitacao, perfil: PerfilEmpresa, documento: str, referencia: str,
    ...
    # Cabeçalho: logo + razão social/CNPJ em azul Finnet
    if _LOGO.is_file():
    ...
    r = p_empresa.add_run(perfil.razao_social or "FINNET S/A - TECNOLOGIA E INSTITUIÇÃO DE PAGAMENTO")
    r.bold = True
    r.font.size = Pt(12)
    r.font.color.rgb = AZUL_FINNET
    ...
    nome = perfil.representante_nome or "Representante Legal"
    cargo = perfil.representante_cargo or ""
