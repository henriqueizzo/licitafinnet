...
# Colunas adicionadas à tabela `analises` (análise das duas frentes Finnet).
...
def migrar_esquema() -> list[str]:
    ...
    with engine.begin() as conn:
        ...
        # Limpa valores corrompidos (U+FFFD) herdados de migrações antigas no cargo
        # do representante; o valor correto é preenchido pelo usuário na aba Perfil.
        if "perfil_empresa" in tabelas:
            res = conn.execute(
                text(
                    "UPDATE perfil_empresa SET representante_cargo = '' "
                    "WHERE representante_cargo LIKE '%' || :fffd || '%'"
                ),
                {"fffd": "�"},
            )
            if res.rowcount:
