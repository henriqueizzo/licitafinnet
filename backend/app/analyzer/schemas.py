...
class ResultadoAnalise(BaseModel):
    ...
    score_beneficios: int = Field(
        description="Score final de 0 a 10 para a frente FINNET EDI/VAN (troca de arquivos bancários)"
    )
    score_pagamentos: int = Field(
        description="Score final de 0 a 10 para a frente FINNET PAGAMENTOS (cobrança, conciliação, instituição de pagamento)"
    )
